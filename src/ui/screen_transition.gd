...
##
## Provides tile-based screen transitions with configurable patterns.
## Add as a child of a CanvasLayer to overlay the entire screen.
...
## Performs a wipe transition: captures current screen, runs callback, fades out.
## This is more efficient than transition_full as it only animates once.
...
## Legacy: Performs a full transition (out then in) with callback at midpoint.
...
## Sets how much random jitter to add to the base pattern (0.0 to 1.0).
...
func _set_use_captured_screen(enabled: bool) -> void:
	var mat := _get_shader_material()
	if mat:
		mat.set_shader_parameter("use_captured_screen", enabled)
