...
extends RefCounted
## Polls ALL keyboard partitions and ALL connected
## gamepads for directional and trigger input. Used
## by screens where no specific device config is
## assigned yet (auth, consent).
...
## Prime previous-input state so inputs already
## held when the poller starts are not detected as
## "just pressed" on the first frame.
func prime() -> void:
