...
# Dictionary<StringName, int> - maps device name
# to player_id. Needed because
# _pending_device_configs_by_index shifts on
# removal, invalidating index-based ID lookups.
var _device_name_to_player_id := {}

# Monotonically increasing counter for unique
# lobby player IDs. Prevents ID collisions when
# players are removed and re-added (array indices
# shift, but this counter never decreases).
...
func _check_keyboard_inputs() -> void:
	...
	for i in range(
		InputDeviceManager
			.KEYBOARD_PARTITION_BINDINGS.size()
	):
		var bindings: Dictionary = (
			InputDeviceManager
				.KEYBOARD_PARTITION_BINDINGS[i])
...
func _check_gamepad_inputs() -> void:
	...
	for device_id in Input.get_connected_joypads():
		...
		if Input.is_action_pressed(
			...
			_try_register_gamepad_player(
				device_id)
		elif Input.is_action_pressed(
			"move_down", device_id
		):
			_try_deregister_gamepad_player(
				device_id)
...
func _try_register_keyboard_player(
	...
	var device_name: StringName = (
		key_bindings["name"])
	if _pending_device_configs_by_name.has(
		device_name
	):
		# Already registered.
		return
...
func _try_register_gamepad_player(
	device_id: int,
) -> void:
	if (
		_pending_device_configs_by_index.size()
...
func _register_player(
	...
	# Use monotonic counter for unique player
	# identity. Dense array index is only for
	# array operations.
	var lobby_id := _next_lobby_id_counter
	_next_lobby_id_counter += 1

	# Register device config BEFORE creating/
	# adding player, so that Player._ready() can
	# find the device when setting up action
	# sources.
	_pending_device_configs_by_index.append(
		device_config)
	_pending_device_configs_by_name[
		device_config.name] = device_config

	# Use lobby_id as the device map key (matches
	# what Player._ready() derives from
	# player_id).
	(G.input_device_manager
		.assign_device_to_player(
			lobby_id, device_config))

	# Now create and add player to tree (triggers
	# _ready()).
	var player: Player = (
		G.settings.default_player_scene
			.instantiate())
	player.player_id = (
		get_local_player_id(lobby_id))
	...
	# Disable inter-player physics collision in
	# lobby. Only remove from mask (don't push
	# other players), keep layer so Area2Ds
	# (RabbitHole) can still detect.
	...
	_device_name_to_player_id[
		device_config.name] = player.player_id
	...
	# Create lobby GamePlayerState for display
	# purposes.
...
func _deregister_player(
	...
	if not _pending_device_configs_by_name.has(
		device_name
	):
		# Not registered.
		return
	...
	var device_config: DeviceConfig = (
		_pending_device_configs_by_name[
			device_name])
	var dense_index := (
		_pending_device_configs_by_index
			.find(device_config))

	# Look up player_id from stable mapping (not
	# from array index, which shifts on removal).
	var player_id: int = (
		_device_name_to_player_id[device_name])
	if not Netcode.ensure(
		players_by_id.has(player_id)
	):
		return

	var player: Player = players_by_id[player_id]

	# Derive the stable lobby_id that was used as
	# the device map key (reverse of
	# get_local_player_id).
	var lobby_id := - (player_id + 1)

	_pending_device_configs_by_index.remove_at(
		dense_index)
	_pending_device_configs_by_name.erase(
		device_name)
	_device_name_to_player_id.erase(device_name)
	(G.input_device_manager
		.unassign_device_from_player(lobby_id))

	# Remove corresponding player attributes.
	(G.client_session.local_player_attributes
		.remove_at(dense_index))
...
## Called by GamePanel to start match with
## current players.
func start_match() -> void:
	...
	# Re-key device assignments from
	# lobby_id-based keys to dense match indices
	# (0, 1, 2...) so the match can look up
	# devices by local_player_index.
	G.input_device_manager.clear_all_assignments()
	for i in range(
		G.client_session
			.local_device_configs.size()
	):
		(G.input_device_manager
			.assign_device_to_player(
				i,
				G.client_session
					.local_device_configs[i],
			))
...
## Re-register players from a previous match,
## preserving their device configs and attributes
## (except color).
...
## previous match with the crown. Called deferred
## so it runs after player appearance is applied.
...
static func get_local_player_id(
	...
	# Lobby players use negative IDs:
	# -1, -2, -3, etc.
...
## Assigns colors to all lobby players based on
## count.
func _update_lobby_colors() -> void:
	...
	var colors := (
		PlayerAttributeGenerator
			.calculate_outline_colors(
				player_ids.size()))
	for i in range(player_ids.size()):
		var player_id: int = player_ids[i]
		if G.match_state.players_by_id.has(
			player_id
		):
			G.match_state.players_by_id[
