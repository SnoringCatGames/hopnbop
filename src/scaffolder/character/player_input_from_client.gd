...
func _get_configuration_warnings() -> PackedStringArray:
    ...
    var forwarded_input: ForwardedPlayerInputFromServer = _find_forwarded_input_sibling()
    if forwarded_input == null:
        ...
        var property_mismatch := _validate_synced_properties_match(forwarded_input)
...
func _reconcile_jump_event() -> void:
    ...
    G.print(
        "F:%d Reconciling jump event from frame %d" % [
    ...
    if not has_jump_bit:
        ...
        # FIXME: Remove after testing.
        G.print(
            "F:%d Jump bit injected into frame %d, queuing rollback" % [
                G.network.server_frame_index,
                jump_frame,
            ],
            ScaffolderLog.CATEGORY_NETWORK_SYNC,
