...
## Callers should use `get_current_legal_version()` instead of
## referencing this constant directly so they pick up server-side
## bumps without a client rebuild.
##
...
static func get_current_legal_version() -> String:
