...
class TestAuthorityHandling:
    extends GutTest
    ## Tests server vs client authority assignment.
    ...
    func test_authority_id_returns_server_when_server_authoritative():
        ...
        assert_eq(
            ...
            "Server-authoritative should return SERVER_ID (1)",
        )
    ...
    func test_is_client_authoritative_property():
        ...
        assert_false(
            entity.is_client_authoritative,
            "Should not be client authoritative when server authoritative",
        )
    ...
    func test_update_authority_calls_set_multiplayer_authority():
        ...
        # Authority should be set to SERVER_ID (1)
        assert_eq(
            entity.get_multiplayer_authority(),
            NetworkConnector.SERVER_ID,
            "Should set authority to SERVER_ID for server-authoritative",
        )
