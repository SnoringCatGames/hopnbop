...
class TestThreeNodePartnerDiscovery:
    ...
    func test_client_auth_finds_server_auth_partner():
        # Create 3-node setup.
        server_state = TestServerAuthState.new()
        server_state.name = "ServerState"
        server_state.root_path = NodePath(".")
        root_node.add_child(server_state)

        client_state = TestClientAuthState.new()
        client_state.name = "ClientState"
        client_state.root_path = NodePath(".")
        root_node.add_child(client_state)

        forwarded_state = TestForwardedState.new()
        forwarded_state.name = "ForwardedState"
        forwarded_state.root_path = NodePath(".")
        root_node.add_child(forwarded_state)

        # Initialize nodes.
        server_state._ready()
        client_state._ready()
        forwarded_state._ready()

        # Client-auth should discover server-auth sibling.
        assert_not_null(
            client_state.state_from_server,
            "Client auth should find state_from_server",
        )
        assert_eq(
            client_state.state_from_server,
            server_state,
            "Client auth should discover server auth state",
        )


    func test_server_auth_finds_client_auth_partner():
        # Create 3-node setup.
        server_state = TestServerAuthState.new()
        server_state.name = "ServerState"
        server_state.root_path = NodePath(".")
        ...
        # Server-auth should discover client-auth sibling.
        assert_not_null(
            server_state.input_from_client,
            "Server auth should find input_from_client",
        )
        assert_eq(
            server_state.input_from_client,
            client_state,
            "Server auth should discover client auth state",
        )


    func test_forwarded_state_finds_server_auth_partner():
        # Create 3-node setup.
        server_state = TestServerAuthState.new()
        server_state.name = "ServerState"
        server_state.root_path = NodePath(".")
        ...
        # Forwarded state should discover server-auth sibling.
        assert_not_null(
            forwarded_state.state_from_server,
            "Forwarded state should find state_from_server",
        )
        assert_eq(
            forwarded_state.state_from_server,
            server_state,
            "Forwarded state should discover server auth state",
        )
...
class TestBackwardCompatibility:
    ...
    func test_two_node_player_now_invalid():
        # 2-node player setup is no longer valid.
        var server_state = TestServerAuthState.new()
        server_state.name = "ServerState"
        server_state.root_path = NodePath(".")
        root_node.add_child(server_state)
        ...
        # Both should still discover each other as siblings.
        assert_eq(
            server_state.input_from_client,
            client_state,
            "Server should discover input_from_client",
        )
        assert_eq(
            client_state.state_from_server,
            server_state,
            "Client should discover state_from_server",
        )

        # Should have warnings - 2-node is no longer valid.
        assert_ne(
            server_state._partner_state_configuration_warning,
            "",
            "2-node setup should have configuration warning",
        )
        assert_true(
            "2-node configuration is no longer valid" in \
            server_state._partner_state_configuration_warning,
            "Warning should mention 2-node is invalid",
        )
