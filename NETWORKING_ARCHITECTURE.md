...
**Jump 'n Thump** - Comprehensive Technical Reference
...
3. [Player Identity & Lifecycle](#3-player-identity--lifecycle)
4. [Frame-Synchronous Simulation](#4-frame-synchronous-simulation)
5. [Rollback Reconciliation](#5-rollback-reconciliation)
6. [Clock Synchronization (NTP)](#6-clock-synchronization-ntp)
7. [Multi-Player Per Client](#7-multi-player-per-client)
8. [GameLift Integration](#8-gamelift-integration)
9. [Pause & Synchronization](#9-pause--synchronization)
10. [State Replication Patterns](#10-state-replication-patterns)
11. [Debugging & Observability](#11-debugging--observability)
...
This document provides a comprehensive technical reference for Jump 'n Thump's networking architecture. It covers the complete system from player ID assignment through rollback reconciliation to AWS GameLift integration.

**Target Audience:** Intermediate to advanced game developers with:
- Basic understanding of client-server architecture
- Familiarity with Godot's node system and multiplayer API
- No prior knowledge of rollback netcode, NTP, or GameLift required

### Relationship to CLAUDE.md

This document complements `CLAUDE.md` but serves a different purpose:

| CLAUDE.md | NETWORKING_ARCHITECTURE.md |
|-----------|----------------------------|
| High-level patterns | Deep technical details |
| Development workflow | System architecture |
| Testing framework (GUT) | Networking algorithms |
| Code style guidelines | Implementation specifics |
| Quick reference | Comprehensive reference |

**When to use this document:**
- Onboarding new developers to the networking systems
- Debugging multiplayer issues (rollback, prediction, sync)
- Understanding player ID assignment and multi-player support
- Implementing new networked features
- Integrating with AWS GameLift

**When to use CLAUDE.md:**
- Setting up the project
- Running tests with GUT framework
...
**Core Topics:**
1. **Player ID System** - Server-assigned sequential integers (1, 2, 3...) with support for multiple players per client
2. **Multi-Player Per Client** - Local multiplayer (couch co-op) with distinct input devices per player
3. **Frame-Based Rollback** - Client-side prediction with server reconciliation at 60 FPS
4. **AWS GameLift Integration** - Player session validation, matchmaking, and server lifecycle management

**Supporting Systems:**
- NTP-like clock synchronization
- Pause/unpause mechanics with frame continuity
- State replication patterns (MatchState, kills, bumps)
...
- **How player IDs work** → Section 3 (Player Identity & Lifecycle)
- **Rollback and prediction** → Sections 4-5 (Frame-Synchronous Simulation, Rollback Reconciliation)
- **Multiple local players** → Section 7 (Multi-Player Per Client)
- **GameLift integration** → Section 8 (GameLift Integration)
- **Why rollback triggered** → Section 11 (Debugging & Observability)
- **Quick reference lookups** → Section 14 (Reference Tables)

**I'm debugging...**

- **Rollback issues** → Sections 5, 11
- **Player spawning problems** → Sections 3, 7
- **Time sync drift** → Section 6
- **GameLift validation failures** → Section 8
- **Test failures** → Section 13

---

## 2. Core Architecture Concepts

### 2.1 The Three-Tier Identity System

Jump 'n Thump's networking architecture uses a three-tier identity system to uniquely identify players, track ownership, and manage local multiplayer:

#### Tier 1: Player ID (Globally Unique)
```gdscript
var player_id: int = 0  # Sequential: 1, 2, 3, 4, 5...
```

- **Assigned by:** Server, via `NetworkConnector._next_player_id` counter
- **Scope:** Globally unique across entire game session
- **Format:** Sequential positive integers starting from 1
- **Purpose:** Primary identifier for all gameplay systems (kills, bumps, scores)
- **Persistence:** Lasts for duration of player's connection

**Example:**
```
Server state:
  player_id=1 → "Fuzzy Bunny"
  player_id=2 → "Soft Hopper"
  player_id=3 → "Hard Thumper"
```

#### Tier 2: Peer ID (Client Machine)
```gdscript
var peer_id: int = 0  # Godot multiplayer API unique ID
```

- **Assigned by:** Godot's multiplayer API when client connects
- **Scope:** Unique per connected client machine
- **Format:** Positive integers (typically 2, 3, 4...; server is always 1)
- **Purpose:** Track which client machine owns which players
- **Persistence:** Lasts for duration of peer's connection

**Example:**
```
Network topology:
  peer_id=1 → Server (never has players)
  peer_id=2 → Client A's machine
  peer_id=3 → Client B's machine
```

#### Tier 3: Local Player Index (Within Peer)
```gdscript
var local_player_index: int = 0  # 0, 1, 2... (0-based)
```

- **Assigned by:** Server, based on order of players declared by peer
- **Scope:** Unique within a single peer
- **Format:** Zero-based index (0 = first player, 1 = second, etc.)
- **Purpose:** Map players to input devices (gamepad 0, gamepad 1, keyboard)
- **Persistence:** Lasts for duration of player's connection

**Example:**
```
Peer 2 declares 2 players:
  player_id=1, peer_id=2, local_player_index=0  # First player on peer 2
  player_id=2, peer_id=2, local_player_index=1  # Second player on peer 2

Peer 3 declares 1 player:
  player_id=3, peer_id=3, local_player_index=0  # First player on peer 3
```

#### Complete Identity Triple

Every player is uniquely described by this triple:

```
(player_id, peer_id, local_player_index)

Examples:
  (1, 2, 0) → Player 1 is the first player on peer 2
  (2, 2, 1) → Player 2 is the second player on peer 2
  (3, 3, 0) → Player 3 is the first player on peer 3
```

This triple is stored in `PlayerMatchState` and replicated to all clients.
...
│                                                              │
│  ┌──────────────────┐  ┌──────────────────┐  ┌───────────┐ │
│  │ ServerTimeTracker│  │ NetworkConnector │  │ NetworkFr │ │
│  │                  │  │                  │  │ ameDriver │ │
│  │ • NTP clock sync │  │ • ENet peer mgmt │  │           │ │
│  │ • 4-timestamp    │  │ • Player ID      │  │ • 60 FPS  │ │
│  │   algorithm      │  │   assignment     │  │   tick    │ │
│  │ • Offset calc    │  │ • Peer tracking  │  │ • Rollback│ │
│  │ • Smoothing      │  │ • RPC handling   │  │   coord   │ │
│  └──────────────────┘  └──────────────────┘  └───────────┘ │
│                                                              │
│  All three subsystems coordinate via NetworkMain             │
└─────────────────────────────────────────────────────────────┘
         │                      │                      │
         │                      │                      │
         ▼                      ▼                      ▼
  ┌────────────┐      ┌─────────────────┐    ┌──────────────┐
  │ MatchState │      │ Level/Networked │    │ Reconcilable │
  │ Synchron   │      │ Level           │    │ Networked    │
  │ izer       │      │                 │    │ State nodes  │
  └────────────┘      └─────────────────┘    └──────────────┘
```
...
1. **NetworkMain** (`src/networking/network_main.gd`)
   - Top-level coordinator
   - Singleton accessible via `G.network`
   - Manages lifecycle of three subsystems
   - Provides high-level API for game code

2. **ServerTimeTracker** (`src/networking/server_time_tracker.gd`)
   - NTP-like clock synchronization
   - Four-timestamp algorithm for offset calculation
   - Sliding window smoothing (5 samples)
   - Handles pause/unpause time tracking

3. **NetworkConnector** (`src/networking/network_connector.gd`)
   - ENet peer management (default port 4433)
   - Player ID assignment (server-authoritative)
   - Maintains `_next_player_id` counter
   - Tracks player_id → peer_id and player_id → local_player_index mappings
   - Handles peer connect/disconnect events

4. **NetworkFrameDriver** (`src/networking/network_frame_driver.gd`)
   - Core frame simulation at 60 FPS
   - Increments `server_frame_index` on each physics tick
   - Manages rollback buffer and reconciliation
   - Coordinates three-phase frame processing across all networked entities

### 2.3 Authority Models

Jump 'n Thump uses a hybrid authority model where different node types have different authorities:

#### Server-Authoritative Nodes

**Authority:** `SERVER_ID` (peer_id = 1)

**Nodes:**
- `CharacterStateFromServer` - Physics state (position, velocity, on_floor)
- `ForwardedPlayerInputFromServer` - Replicated input from server to all clients

**Why:** Game logic and physics must be deterministic and authoritative. The server is the source of truth for:
- Character positions
- Collision states
- Game state (kills, bumps, scores)

#### Client-Authoritative Nodes

**Authority:** Owning peer's `peer_id` (2, 3, 4...)

**Nodes:**
- `PlayerInputFromClient` - Local input capture (jump, move_left, move_right)

**Why:** Only the owning client knows its input. This node captures local input and replicates it to the server.

#### Example: 3-Node Player Architecture

For a player owned by peer_id=2:

```
Bunny (Player node)
│
├─ CharacterStateFromServer         [Authority: 1 (server)]
│  │
│  ├─ position: Vector2             ← Replicated from server
│  ├─ velocity: Vector2             ← Replicated from server
│  ├─ is_on_floor: bool             ← Replicated from server
│  └─ player_id: int = 5            ← Set by server at spawn
│
├─ PlayerInputFromClient            [Authority: 2 (owning peer)]
│  │
│  └─ input_bitmask: int            ← Replicated from client to server
│
└─ ForwardedPlayerInputFromServer   [Authority: 1 (server)]
   │
   └─ input_bitmask: int            ← Replicated from server to other clients
```

**Flow:**
1. Peer 2 captures input → `PlayerInputFromClient.input_bitmask` = 0b0010 (jump pressed)
2. Godot replicates to server (because PlayerInputFromClient has authority=2)
3. Server processes input in `_network_process()` → updates `CharacterStateFromServer`
4. Server replicates state to all clients (because CharacterStateFromServer has authority=1)
5. Server forwards input to other clients via `ForwardedPlayerInputFromServer`

**Why three nodes instead of two?**

- **Separation of concerns:** Input capture vs input forwarding
- **Authority isolation:** Client can only modify its own input, not forwarded input
- **Visibility filtering:** `ForwardedPlayerInputFromServer` can hide from originating peer (reduces bandwidth)

### 2.4 Frame-Based Timekeeping

All networked simulation is driven by **server frame indices**, not wall-clock time.

```gdscript
# NetworkFrameDriver
var server_frame_index: int = 0  # Canonical time
TARGET_NETWORK_FPS = 60
TARGET_NETWORK_TIME_STEP_SEC = 1.0 / 60  # 0.0166...
```

**Key Principles:**

1. **Server frame index is authoritative**
   - Incremented directly on each `_physics_process` call
   - Never skips (even during pause, pause_frame is recorded separately)
   - All game logic references frame_index, not timestamps

2. **60 FPS fixed timestep**
   - Network simulation always runs at 60 FPS
   - Independent of render framerate
   - Deterministic physics stepping

3. **Frame time calculated from index**
   ```gdscript
   server_frame_time_usec = (
       server_frame_index * TARGET_NETWORK_TIME_STEP_USEC
       + _start_time_offset_usec
       - _cumulative_pause_time_usec
   )
   ```

4. **State snapshots indexed by frame**
   - Rollback buffer stores state at each frame_index
   - Client-side prediction references future frames
   - Mismatch detection compares states at same frame_index

**Why frame-based instead of time-based?**

- Eliminates clock drift issues
- Deterministic simulation
- Rollback "rewinds" to specific frames, not timestamps
- Easier debugging (frame 1234 vs 20543.215ms)

---

## 3. Player Identity & Lifecycle

This section describes how player IDs are assigned, how the identity triple works, and the complete player lifecycle from connection to disconnection.

### 3.1 Player ID Assignment Flow

The player ID assignment process involves a multi-step RPC exchange between client and server.

**Complete Flow Example:**

```
Peer 2 connects and declares 2 players → Server assigns IDs [1, 2]
Peer 3 connects and declares 1 player  → Server assigns ID [3]
Peer 4 connects and declares 3 players → Server assigns IDs [4, 5, 6]
```

#### Step-by-Step Process

**Step 1: Client Connects via ENet**

The client establishes a connection to the server using default port 4433.

**Step 2: Client Declares Players**

After connection established, the client calls:

```gdscript
# NetworkConnector (client sends RPC)
_server_rpc_declare_players.rpc_id(1, session_ids)
```

Where `session_ids` is an array of GameLift player_session_id strings (or empty in preview mode).

**Step 3: Server Assigns Sequential Player IDs**

```gdscript
# src/networking/network_connector.gd (Server receives RPC)
...
**Step 4: Client Receives Assigned IDs**
...
**Step 5: Server Creates PlayerMatchState and Spawns Characters**

The `peer_players_declared` signal triggers two handlers:

1. **MatchStateSynchronizer** creates `PlayerMatchState` for each player
2. **NetworkedLevel** spawns `Player` character nodes

These run in parallel, both triggered by the same signal.

#### Sequence Diagram

```
Client                    Server                  MatchState      NetworkedLevel
  |                         |                         |                 |
  |--[1] ENet connect------>|                         |                 |
  |<-----connected----------|                         |                 |
  |                         |                         |                 |
  ...
  |                         |                         |   Spawn chars   |
  |                         |                         |                 |
  |<--[6] MultiplayerSpawner replicates player nodes------------------|
  |                         |                         |                 |
```

### 3.2 LocalSession: Client-Side State

`LocalSession` stores per-client configuration and assigned player IDs:

```gdscript
# src/core/local_session.gd
class_name LocalSession

var local_player_count: int = 0
var local_device_configs: Array[DeviceConfig] = []
var local_session_ids: Array[String] = []
var local_player_ids: Array[int] = []  # Assigned by server
```

**Example state for 2-player local co-op:**

```gdscript
local_player_count = 2
local_device_configs = [
    DeviceConfig(GAMEPAD, 0),  # Player 1 uses gamepad 0
    DeviceConfig(GAMEPAD, 1)   # Player 2 uses gamepad 1
]
local_session_ids = ["session-abc", "session-def"]  # From GameLift
local_player_ids = [1, 2]  # Assigned by server
```

### 3.3 PlayerMatchState: Replicated Metadata

`PlayerMatchState` stores per-player metadata replicated to all clients:

```gdscript
# src/core/player_match_state.gd
var player_id: int = 0
var peer_id: int = 0
var local_player_index: int = 0
var bunny_name: String = ""
var adjective: String = ""
var is_soft: bool = true
var connect_time_usec: int = 0
var disconnect_time_usec: int = 0
```

Stored in `MatchState.players_by_id` dictionary and replicated via packed array.

### 3.4 Derived Properties in ReconcilableNetworkedState

The `peer_id` and `local_player_index` are derived from `PlayerMatchState` lookup:
...
var peer_id: int:
    get:
        ...
        if player_id != 0 and is_instance_valid(G.match_state):
            var player_state := G.match_state.players.get(player_id)
...
- Single source of truth (`PlayerMatchState`)
- No duplicate storage or sync issues
- Fallback for tests and editor mode
