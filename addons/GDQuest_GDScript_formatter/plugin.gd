...
func _on_resource_saved(saved_resource: Resource) -> void:
	...
	if format_on_save:
		...
		if script_editor.get_current_script() == script:
			reload_code_edit(
				script_editor.get_current_editor().get_base_editor(), formatted_code, true
			)
...
func lint_code(script: GDScript) -> Array:
	...
	var exit_code := OS.execute(
		get_editor_setting(SETTING_FORMATTER_PATH), formatter_arguments, output
	)
	if exit_code == OK:
...
class CodeEditState:
	...
	func restore_to_editor(code_edit: CodeEdit) -> void:
		...
		_restore_line_features(
			folds,
			func(line: int, _is_folded: bool) -> void: code_edit.fold_line(line),
			new_line_count
		)
	...
	func _restore_line_features(
		...
		for line_index in range(stored_lines.size()):
			...
			while line_above >= 0 or line_below < new_line_count:
				if (
					line_below < new_line_count
					and code_edit.get_line(line_below).similarity(original_text) > 0.9
				):
					set_line_func.call(line_below, true)
					break
				if (
					line_above >= 0
					and code_edit.get_line(line_above).similarity(original_text) > 0.9
				):
